...
The primary rubric decides a work's form: `Poezija` is verse, `Slikarstvo` a gallery and every other rubric formatted prose. Choosing a rubric converts pasted text without losing words or bold/italic; opening an older work saved as verse outside Poezija (or the reverse) realigns its draft, which then needs **Objavi izmjene**.

Every published text has a **Podijeli** button (Facebook, Viber, copy link, and the phone's share sheet) that shares the canonical article URL; editors get the same links after publishing. Link previews use the work's first picture as a 1200×630 JPEG card at `/media/{id}/share.jpg`, generated once beside the other derivatives and served under the same publication rules as the picture itself.

Homepage placements are `Glavni tekst`, `Izbor poezije`, and `Umjetnost`, with recent-work fallbacks and no adjacent duplication. Choices work even when a selected work is older than the first archive page. Publishing and moderation are immediately reflected in server-rendered pages without a rebuild. Public document responses use no-store caching. Pictures are `private, no-cache` with an ETag: browsers keep them but ask again on every view, the access check runs each time, and an unchanged picture answers 304, so withdrawn pictures stop being served at once. Static versioned fonts/assets retain framework caching. This intentionally favors correctness for a small publication over a shared content cache.
